...
#if defined(NDEBUG)
# error "Craigcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa72915603469b20fd611ecc60d8770452c0b3bdbf31f334932928115aa30fdc4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Craigcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Craigcoin Signed Message:\n";
...
{
    // Craigcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 533 * COIN;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60; // Craigcoin: 1 minute
static const int64 nTargetSpacing = 1 * 60; // Craigcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Craigcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Craigcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa72915603469b20fd611ecc60d8770452c0b3bdbf31f334932928115aa30fdc4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 07/Mar/2016 Donald Trump Florida Ad Hits ‘Corrupt Marco Rubio’";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047351910fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1457327465;
        ...
        assert(block.hashMerkleRoot == uint256("0x05f46743046e7a306c6f8a94763968c8cd6b2df9bf36e152d40dae89496f0895"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb4, 0xde }; // Craigcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CraigcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CraigcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CraigcoinMiner(CWallet *pwallet)
{
    printf("CraigcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("craigcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CraigcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CraigcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CraigcoinMiner, pwallet));
}
